...
HandwriteInput.prototype.updateStream = function (prev, current) {
  if (prev) {
    ...
    while (i != current.x) {
      this.stream[i] =
        prev.y + (current.y - prev.y) * ((i - prev.x) / (current.x - prev.x));
...
Renderer.prototype.renderStream = function (stream, bounds, label) {
  ...
  if (label) {
    ...
    this.context.fillText(label, bounds.x - 70, bounds.y + bounds.height / 2);
  }
};
...
Renderer.prototype.renderWarpingPath = function (warpingPath, bounds, label) {
  ...
  if (label) {
    this.context.textBaseline = "middle";
    this.context.fillText(label, bounds.x - 80, bounds.y + bounds.height / 2);
  }
};
...
Renderer.prototype.renderWarpingPathHighlight = function (highlight, bounds) {
  ...
  this.context.lineTo(
    bounds.x + highlight[1][0] * 1,
    bounds.y + bounds.height - margin
  );
  this.context.lineTo(
    bounds.x + highlight[1][1] * 1,
    bounds.y + bounds.height - margin
  );
...
function init() {
  ...
  var update = function (e) {
    ...
    renderer.renderStream(
      stream1,
      { x: 100, y: 10, width: 400, height: 100 },
      "stream (x)"
    );
    renderer.renderStream(
      stream2,
      { x: 100, y: 210, width: 400, height: 100 },
      "stream (y)"
    );
    renderer.renderWarpingPath(
      w,
      { x: 100, y: 110, width: 400, height: 100 },
      "warping path (w)"
    );
